...
lint: ## Lint all yaml files for a given INSTANCE
	find ./$(INSTANCE) -name '*.yml' | grep '^\./[^/]*/' | xargs -n 1 python3 scripts/yaml_check.py
	find ./$(INSTANCE) -name '*.yml' | grep '^\./[^/]*/' | xargs -n 1 -I{} pykwalify -d '{}' -s .schema.yml
	find ./$(INSTANCE) -name '*.yml' | grep '^\./[^/]*/' | xargs -n 1 python3 scripts/identify_unpinned.py
...
fix: ## For a given INSTANCE fix all lockfiles and add the latest revision to every repo that has no revision
	...
	find ./$(INSTANCE) -name '*.yml' | grep '^\./[^/]*/' | xargs -n 1 python3 scripts/fix_lockfile.py
	@# --without says to add the latest revision to every entry missing one (i.e. update all)
	find ./$(INSTANCE) -name '*.yml' | grep '^\./[^/]*/' | xargs -n 1 python3 scripts/update_tool.py --without --log debug
